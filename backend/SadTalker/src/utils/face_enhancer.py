...
def enhancer_generator_with_len(images, method='gfpgan', bg_upsampler='realesrgan'):
    ...
    gen = enhancer_generator_no_len(images, method=method, bg_upsampler=bg_upsampler)
    gen_with_len = GeneratorWithLen(gen, frame_num)
    return gen_with_len
...
def enhancer_generator_no_len(images, method='gfpgan', bg_upsampler='realesrgan'):
    ...
    print('face enhancer....')
    is_video_path = isinstance(images, str) and os.path.isfile(images)
    video_stream = None
    if is_video_path:
        video_stream = cv2.VideoCapture(images)
        frame_num = int(video_stream.get(cv2.CAP_PROP_FRAME_COUNT))
    ...
    # ------------------------ restore ------------------------
    for idx in tqdm(range(frame_num), 'Face Enhancer:'):
        
        if is_video_path:
            still_reading, frame = video_stream.read()
            if not still_reading:
                break
            img = frame # cv2.VideoCapture reads as BGR
        else:
    ...
    if is_video_path and video_stream is not None:
        video_stream.release()
